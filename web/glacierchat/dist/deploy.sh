...
build_container() {
  ...
  docker build -t localhost/chall-blindsql --platform linux/amd64 . 
}
...
run_container() {
  ...
  docker run --name chall-blindsql --rm -p 1337:80 -it localhost/chall-blindsql
}

kill_container() {
	docker ps --filter "name=chall-blindsql" --format "{{.ID}}" \
		| tr '\n' ' ' \
