...
# TODO: size of tmpfs
podman run -d --uts=private --read-only --tmpfs /tmp:rw,size=16M --hostname "${HOSTNAME}" --timeout ${TIMEOUT} --cidfile=${DIR}/cid -p "127.0.0.1:${PORT}:1337" --network=pasta:-i,lo,-I,hostlo ${REGISTRY}/${NAME}-challenge 2>/dev/null 1>/dev/null
